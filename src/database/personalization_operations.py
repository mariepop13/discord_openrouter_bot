...
async def get_personalization(user_id: int) -> Optional[Tuple[Any, ...]]:
    logging.debug(f"Fetching personalization for user_id: {user_id}")
    try:
        result = await execute_query('SELECT personality, tone, language, ai_model, max_output FROM personalization WHERE user_id = ?',
                                     (user_id,), fetchone=True)
        logging.debug(f"Personalization fetched for user_id: {user_id}, result: {result}")
        return result
...
async def set_personalization(user_id: int, field: str, value: str) -> None:
    logging.debug(f"Setting personalization for user_id: {user_id}, field: {field}, value: {value}")
    try:
        await execute_query(f'INSERT OR REPLACE INTO personalization (user_id, {field}) VALUES (?, ?)',
                            (user_id, value))
        logging.debug(f"Personalization set for user_id: {user_id}, field: {field}, value: {value}")
    except Exception as e:
